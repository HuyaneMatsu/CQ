...
class quest(object):
    def __init__(self,*args):
        ...
        for value in potential:
            for subv in value._rq.stage:
                if is_raid(subv):
                    add('raid')
                elif is_colosseum(subv):
                    add('colosseum')
                else:
                    add(get_stage_chapter_id(subv))
...
quest_chapter_group_names=['Colosseum','Raid']+chapter_name_titles
quest_chapter_group_metas=['colosseum','raid']+['N-'+str(i) for i in range(1,5)]+['C']
...
def createdocument():
    ...
    write('''<a class='title'>Matsu's quest-thingy</a></br></br></br>Thanks for help for:<ul><li>Shiki for raid/colo quests</li><li>BiBi/BrainLord for C3/4 quests</li><li>Narwhal for being Kitty Cat</li></ul></br></br>If u find any missing quests, please report them, so I can add them.",''')
    ...
    for v in (quest_type_names,quest_type_metas,quest_reward_names,quest_reward_metas,quest_chapter_group_names,quest_chapter_group_metas):
        write(str(v))
        write(',')
    write(''');''')
    #end
